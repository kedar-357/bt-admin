      ...
      return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center justify-between">
            <Button
              variant="ghost"
              className="pl-0 text-gray-500 hover:text-bt-indigo dark:text-slate-400 transition-colors"
          ...
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-3xl font-extrabold text-bt-text dark:text-white mb-2">
                Finalizing Quote {selectedQuote.id}
              </h2>
              <p className="text-gray-500 dark:text-slate-400">
                Performing mandatory pre-order checks for{' '}
                <span className="text-bt-indigo dark:text-bt-indigoLight font-bold">
            ...
            <div className="space-y-6">
              ...
              <Card className="border-l-4 border-bt-success shadow-soft">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <span className="inline-block mt-2 text-xs font-bold text-green-700 bg-green-50 px-2 py-1 rounded dark:bg-green-900/30 dark:text-green-400">
                      Completed
                    </span>
                  </div>
                </div>
              ...
              <Card
                className={clsx(
                  'border-l-4 shadow-soft transition-all duration-500',
                ...
                <div className="flex items-start">
                  <div
                    className={clsx(
                      ...
                      isCreditChecking
                        ? 'bg-indigo-100 text-bt-indigo dark:bg-indigo-900/50 dark:text-bt-indigoLight'
                  ...
                  <div className="flex-1">
                    <h4 className="text-lg font-bold text-gray-900 dark:text-white flex items-center justify-between">
                      Credit Bureau Check
                      {isCreditChecking && (
                        <span className="text-xs font-bold text-bt-indigo dark:text-bt-indigoLight uppercase animate-pulse">
                          In Progress...
                        </span>
                      )}
                    </h4>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                      Verifying financial profile with UK Credit Bureau.
                    </p>
                    {isCreditChecking && (
                      <pre className="mt-4 p-3 bg-white dark:bg-bt-darkCard rounded-lg border border-indigo-100 dark:border-slate-700 text-xs text-gray-500 dark:text-slate-400 font-mono whitespace-pre-wrap">
{`> Connecting to Equifax/Experian API...
> Retrieving credit score for ${selectedQuote.customerName}...
> Assessing risk factors...`}
              ...
              <Card
                className={clsx(
                  'border-l-4 shadow-soft transition-all duration-500',
                  isSiteChecking
                    ? 'border-bt-indigo bg-indigo-50/50 dark:bg-indigo-900/10'
                    : isCreditChecking
              ...
              >
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                      Checking fibre availability at <span className="font-semibold">{selectedQuote.site}</span>.
                    </p>
                    {isSiteChecking && (
                      <pre className="mt-4 p-3 bg-white dark:bg-bt-darkCard rounded-lg border border-indigo-100 dark:border-slate-700 text-xs text-gray-500 dark:text-slate-400 font-mono whitespace-pre-wrap">
{`> Querying Openreach Infrastructure Database...
> Validating NAD key for address...
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-slate-400 text-lg">
              Sales configuration for <span className="text-bt-indigo dark:text-bt-indigoLight font-semibold">{selectedQuote.site}</span>
            </p>
          </div>
            ...
            <thead className="bg-gray-50/50 dark:bg-slate-800/50">
              <tr>
                {['Quote ID', 'Customer', 'Site', 'Created', 'Value', 'Status', 'Owner', ''].map(h => (
                  <th key={h} className="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white dark:bg-bt-darkCard divide-y divide-gray-50 dark:divide-slate-800">
              {filteredQuotes.length === 0 ? (
                ...
                filteredQuotes.map(q => (
                  ...
                  >
                    <td className="px-6 py-5 whitespace-nowrap font-bold text-bt-indigo dark:text-bt-indigoLight group-hover:underline">
                      {q.id}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm font-semibold text-bt-text dark:text-slate-200">
                      {q.customerName}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500 dark:text-slate-400 font-medium">
                      {q.site}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-400 font-medium">{q.createdDate}</td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm font-bold text-bt-text dark:text-slate-100">
                      £{q.value.toLocaleString('en-GB', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <StatusChip status={q.status} />
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500 font-medium dark:text-slate-400">
                      {q.owner}
                    </td>
                    ...
                    <div>
                      ...
                      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {lineItems.map((item, idx) => (
                          <div
                            key={idx}
                            className="flex gap-4 items-end bg-gray-50 dark:bg-slate-800/50 p-4 rounded-xl border border-gray-100 dark:border-slate-700 relative group transition-all hover:bg-white dark:hover:bg-slate-800 hover:border-bt-indigo/20 hover:shadow-sm"
                          >
                    ...
                    <div className="flex items-center space-x-4 bg-green-50 dark:bg-green-900/30 p-4 rounded-xl border border-green-100 dark:border-green-800">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-700 dark:text-green-500">
                          Please verify the quote details below before final submission.
                        </p>
                      </div>
                    </div>
